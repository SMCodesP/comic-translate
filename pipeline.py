...
class ComicTranslatePipeline:
        ...
        if self.main_page.image_viewer.hasPhoto() and blk_list:
            for blk in blk_list:
                ...
                rect_item.signals.rectangle_changed.connect(
                    self.main_page.handle_rectangle_change
                )
                self.main_page.image_viewer._rectangles.append(rect_item)

            rect = self.main_page.find_corresponding_rect(
                self.main_page.blk_list[0], 0.5
            )
    ...
    def inpaint_and_set(self):
        if (
            self.main_page.image_viewer.hasPhoto()
            and self.main_page.image_viewer.has_drawn_elements()
        ):
            image = self.main_page.image_viewer.get_cv2_image()
    ...
    def OCR_image(self):
        ...
        if (
            self.main_page.image_viewer.hasPhoto()
            and self.main_page.image_viewer._rectangles
        ):
            image = self.main_page.image_viewer.get_cv2_image()
            # Print block length
    ...
    def translate_image(self):
        ...
        if self.main_page.image_viewer.hasPhoto() and self.main_page.blk_list:
            ...
            format_translations(
                self.main_page.blk_list, trg_lng_cd, upper_case=upper_case
            )
    ...
    def skip_save(
        ...
        cv2.imwrite(
            os.path.join(path, f"{base_name}_translated{extension}"), image_save
        )

    def log_skipped_image(self, directory, timestamp, image_path):
        with open(
            os.path.join(
                directory, f"comic_translate_{timestamp}", "skipped_images.txt"
            ),
            "a",
            encoding="UTF-8",
        ) as file:
    ...
    def batch_process(self):
        ...
        for index, image_path in enumerate(self.main_page.image_files):
            ...
            if self.block_detector_cache is None:
                bdetect_device = (
                    0 if self.main_page.settings_page.is_gpu_enabled() else "cpu"
                )
                self.block_detector_cache = TextBlockDetector(
                    "models/detection/comic-speech-bubble-detector.pt",
                    "models/detection/comic-text-segmenter.pt",
            ...
            cv2.imwrite(
                os.path.join(render_save_dir, f"{base_name}_translated{extension}"),
                rendered_image_save,
            )
        ...
        if archive_info_list:
            ...
            for archive_index, archive in enumerate(archive_info_list):
                ...
                target_lang = self.main_page.image_states[
                    archive["extracted_images"][0]
                ]["target_lang"]
                target_lang_en = self.main_page.lang_mapping.get(
